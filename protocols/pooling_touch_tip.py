...
def run(ctx):
    ...
    if protocol == "pooling":
        source_plate = ctx.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt', 2)
    ...
    dest_wells = dest_tube_rack.wells()
    ...
    if protocol == "pooling":
        ...
        for line in csv_lines:
            ...
            if dna_vol < 20.0:
                p20.pick_up_tip()
                p20.aspirate(dna_vol, source_well.top(z = -19.5))
                p20.dispense(dna_vol, dest_wells[0].top(z = -30))
                p20.blow_out()
                p20.touch_tip(speed=30)
                p20.drop_tip()

            else:
                p20.pick_up_tip()
                for i in range(2):
                    p20.aspirate(dna_vol/2.0, source_well.top(z = -19.5))
                    p20.dispense(dna_vol/2.0, dest_wells[0].top(z = -30))
                    p20.blow_out()
                    p20.touch_tip(speed=30)
                p20.drop_tip()
